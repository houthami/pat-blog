...
import { AlertCircle, Save, Eye, Send, X, Trash2 } from "lucide-react"
...
export default function EditPostForm({ post, site }: EditPostFormProps) {
  ...
  const handleTitleChange = (value: string) => {
    setFormData(prev => ({
      ...prev,
      title: value,
      metaTitle: prev.metaTitle || value.slice(0, 60)
    }))
  }
  ...
  const handleSubmit = async (status?: string) => {
    ...
    try {
      const response = await fetch(`/api/sites/${site.id}/posts/${post.id}`, {
        method: "PUT",
        headers: {
      ...
      if ((status || formData.status) === "PUBLISHED") {
        router.push(`/${site.subdomain}/${updatedPost.slug}`)
      } else {
