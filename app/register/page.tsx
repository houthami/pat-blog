...
export default function RegisterPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result?.error) {
        setError("Google sign-in failed. Please check OAuth configuration.")
      } else if (result?.url) {
  ...
  const handleFacebookSignIn = async () => {
    ...
    try {
      ...
      if (result?.error) {
        setError("Facebook sign-in failed. Please check OAuth configuration.")
      } else if (result?.url) {
        window.location.href = result.url
      }
    } catch (error) {
      ...
      <div className="w-full max-w-md">
        <Card className="shadow-xl border-border/50 backdrop-blur-sm">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl font-bold text-balance">Join Pastry Blog</CardTitle>
              <CardDescription className="text-muted-foreground">
                Create an account to start exploring delicious recipes
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
